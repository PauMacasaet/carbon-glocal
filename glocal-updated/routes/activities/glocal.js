...
router3.get('/', (request, response, next) => {
    pool.query("SELECT activityNo, trackingNo AS glocalId, productName, cp.client, timeIn, timeouts, personName, typeOfActivity, purposeOfVisit, activityPerformed, nextActivity, recommendations, timeIn, timeOuts, assignedSystemsEngineer FROM activities a JOIN contact_person cp ON a.client = cp.client ORDER BY activityNo ASC", (err, res) => {

...
router3.get('/:engid', (request, response, next) => {
    ...
    pool.query('SELECT engid, productName, typeOfActivity, purposeOfVisit, activityPerformed, nextActivity, recommendations, engineerName as engineerSurname FROM activities WHERE engid= ($1)', [engid], (err, res) => {
        if (err) return next(err);
